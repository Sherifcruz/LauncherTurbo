...
final class AppStore: ObservableObject {
    ...
    func configure(modelContext: ModelContext) {
        ...
        $items
            ...
            .sink { [weak self] _ in
                ...
                self.scheduleSaveOrder(delay: 1.0)
            }
            .store(in: &cancellables)
        */
    }
    ...
    /// 调度一次延迟保存（合并多次调用）
    /// - Parameter delay: 延迟时间，默认 1.0 秒；页面切换动画期间会自动再延迟
    func scheduleSaveOrder(delay: TimeInterval = 1.0) {
        // 如果已经有待执行的保存，跳过
        ...
        let workItem = DispatchWorkItem { [weak self] in
            ...
            if self.isPageTransitioning {
                ...
                self.scheduleSaveOrder(delay: 0.5)
                return
            }
            self.isSavePending = false
            self.saveAllOrderNow()
        }
        saveOrderWorkItem = workItem
        DispatchQueue.main.asyncAfter(deadline: .now() + delay, execute: workItem)
    }

    /// 立即执行保存（内部使用，外部应调用 scheduleSaveOrder）
    private func saveAllOrderNow() {
        ...
        print("⏱️ [Save] Starting save for \(items.count) items...")

        // 写入新模型：按页-槽位
        do {
            var stepStart = CFAbsoluteTimeGetCurrent()
            let existing = try modelContext.fetch(FetchDescriptor<PageEntryData>())
            print("⏱️ [Save] Fetch existing: \(String(format: "%.2f", (CFAbsoluteTimeGetCurrent() - stepStart) * 1000))ms (\(existing.count) items)")

            stepStart = CFAbsoluteTimeGetCurrent()
            for row in existing { modelContext.delete(row) }
            print("⏱️ [Save] Delete existing: \(String(format: "%.2f", (CFAbsoluteTimeGetCurrent() - stepStart) * 1000))ms")
            ...
            print("⏱️ [Save] Insert new items: \(String(format: "%.2f", (CFAbsoluteTimeGetCurrent() - stepStart) * 1000))ms")

            stepStart = CFAbsoluteTimeGetCurrent()
            try modelContext.save()
            print("⏱️ [Save] Commit to disk: \(String(format: "%.2f", (CFAbsoluteTimeGetCurrent() - stepStart) * 1000))ms")
            ...
            print("✅ [Save] Total save time: \(String(format: "%.2f", totalTime))ms")
            if totalTime > 16 {
    ...
    func saveAllOrder() {
        // 直接调度延迟保存，避免频繁写入
        scheduleSaveOrder(delay: 0.3)
    }

    /// 强制立即保存（仅用于窗口关闭、导入导出等必须立即生效的场景）
    func forceSaveAllOrder() {
        saveOrderWorkItem?.cancel()
        isSavePending = false
        saveAllOrderNow()
    }
    ...
    func triggerGridRefresh() {
        ...
        print("🔄 [Perf] triggerGridRefresh called - this will rebuild all grids")
    }
